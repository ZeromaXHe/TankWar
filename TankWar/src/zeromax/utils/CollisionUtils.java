    ...
    private static int[] endAfterCollide(Moveable moveable, Facing facing,
        ...
        for (Moveable move : listMove) {
            ...
            if (move instanceof Bullet) {
                if (((Bullet) move).getShotFrom() == moveable) continue;
                else {
...
//                        ((Bullet) move).setToBeCleared(true);
    ...
    private static int[] moveCollideMapArea(Moveable moveable, Facing facing, int distance) {
        int nowi1 = (moveable.getPosX() + 1) / Config.TILEX;//开始的左边x坐标
        int nowj1 = (moveable.getPosY() + 1) / Config.TILEY;//开始时上边y坐标
        int nowi2 = (moveable.getPosX() + moveable.getX() - 1) / Config.TILEY;
        int nowj2 = (moveable.getPosY() + moveable.getY() - 1) / Config.TILEY;
        int endi1 = nowi1;
        int endj1 = nowj1;
        int endi2 = nowi2;
        int endj2 = nowj2;
        switch (facing) {
            case NORTH: {
                endj1 = (moveable.getPosY() - distance) / Config.TILEY;
                endj2 = (moveable.getPosY() - distance + moveable.getY()) / Config.TILEY;
            ...
            case WEST: {
                endi1 = (moveable.getPosX() - distance) / Config.TILEX;
                endi2 = (moveable.getPosX() - distance + moveable.getX()) / Config.TILEX;
    ...
    private static int[] endAfterCollideMap(Moveable moveable, Facing facing, Map map, int distance, int[] collideMapArea) {
        int[] ans = new int[2];
        Drawable d;
        ...
        for (int i = collideMapArea[0];
            ...
            if (i != collideMapArea[0] && i == collideMapArea[1])
                ans[0] = endWithoutCol(moveable, facing, distance);
        }
        return ans;
    }
